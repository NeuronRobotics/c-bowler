...
void pushPIDLimitEvent(PidLimitEvent * event){
	...
	if(event->type == INDEXEVENT){
		...
		//pidReset(event->group,getPosition(event->group));
...
void RunPDVel(BYTE chan){
	if(velData[chan].enabled==TRUE){
		...
		float set = (velData[chan].currentOutputVel+(proportional*pidGroups[chan].K.P)+(velocityDiff*pidGroups[chan].K.D))/-10;
