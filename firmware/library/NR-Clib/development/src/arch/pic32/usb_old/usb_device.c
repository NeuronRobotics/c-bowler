...
void USBStallHandler(void)
{
    ...
    if(U1EP0bits.EPSTALL == 1)
    {
        ...
        if((pBDTEntryEP0OutCurrent->STATUS.Val == _USIE) && (pBDTEntryIn[0]->STATUS.Val == (_USIE|_BSTALL)))
        {
            // Set ep0Bo to stall also
            pBDTEntryEP0OutCurrent->STATUS.Val = _USIE|_DAT0|_DTSEN|_BSTALL;
        }//end if
...
void USBCtrlEPService(void)
{
    ...
    if((USTATcopy & USTAT_EP0_PP_MASK) == USTAT_EP0_OUT_EVEN)
    {
        ...
        if(pBDTEntryEP0OutCurrent->STATUS.PID == PID_SETUP)
        {
...
void USBCtrlTrfSetupHandler(void)
{
    ...
    if(pBDTEntryIn[0]->STATUS.UOWN != 0)
    {
		//give control back to the CPU
		//  Compensate for after a STALL
        pBDTEntryIn[0]->STATUS.Val = _UCPU;
    }
...
void USBCtrlTrfOutHandler(void)
{
    ...
    else //In this case the last OUT transaction must have been a status stage of a CTRL_TRF_TX
    {
		...
		if(BothEP0OutUOWNsSet == FALSE)
		{
	        ...
	        pBDTEntryEP0OutNext->STATUS.Val = _USIE|_DAT0|_DTSEN|_BSTALL;
		}
		else
		{
...
void USBCtrlTrfInHandler(void)
{
    ...
    if(controlTransferState == CTRL_TRF_TX)
    {
        ...
        if(shortPacketStatus == SHORT_PKT_SENT)
        {
            ...
            pBDTEntryIn[0]->STATUS.Val = _USIE|_BSTALL;
        }
        else
        {
            if(lastDTS == 0)
            {
                pBDTEntryIn[0]->STATUS.Val = _USIE|_DAT1|_DTSEN;
            }
            else
            {
                pBDTEntryIn[0]->STATUS.Val = _USIE|_DAT0|_DTSEN;
            }
        }//end if(...)else
    }
        ...
        else
        {
            ...
            *pUEP &= ~UEP_STALL;
            ...
            else
            {
				...
				//  correct
                #if (USB_PING_PONG_MODE == USB_PING_PONG__ALL_BUT_EP0) || (USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG)   
                    p->STATUS.Val = _USIE|_DAT0|_DTSEN;
                    //toggle over the to the next buffer
                    ((BYTE_VAL*)&p)->Val ^= USB_NEXT_PING_PONG;
                    p->STATUS.Val = _USIE|_DAT1|_DTSEN;
                #else
                    p->STATUS.Val = _USIE|_DAT1|_DTSEN;
                #endif
    ...
    switch(SetupPkt.Recipient)
    {
        ...
        case USB_SETUP_RECIPIENT_ENDPOINT_BITFIELD:
             ...
             */
            {
                ...
                if(p->STATUS.Val & _BSTALL)    // Use _BSTALL as a bit mask
...
void USBCtrlEPServiceComplete(void)
{
    ...
    if(inPipes[0].info.bits.busy == 0)
    {
        if(outPipes[0].info.bits.busy == 1)
        {
            ...
            pBDTEntryEP0OutNext->STATUS.Val = _USIE|_DAT1|_DTSEN;
        }
        else
        {
            /*
...
void USBCtrlTrfTxService(void)
{
     ...
     */
    #if defined(__18CXX)
        pBDTEntryIn[0]->STATUS.BC9 = 0;
        pBDTEntryIn[0]->STATUS.BC8 = 0;
    #endif
    
    #if defined(__18CXX) || defined(__C30__)
        pBDTEntryIn[0]->STATUS.Val |= byteToSend.byte.HB;
...
void USBCtrlTrfRxService(void)
{
    ...
    if(outPipes[0].wCount.Val > 0)
    {
        ...
        pBDTEntryEP0OutNext->CNT = USB_EP0_BUFF_SIZE;
        pBDTEntryEP0OutNext->ADR = ConvertToPhysicalAddress(&CtrlTrfData);
        if(pBDTEntryEP0OutCurrent->STATUS.DTS == 0)
        {
            pBDTEntryEP0OutNext->STATUS.Val = _USIE|_DAT1|_DTSEN;
        }
        else
        {
            pBDTEntryEP0OutNext->STATUS.Val = _USIE|_DAT0|_DTSEN;
        }
    }
    else
    {
        ...
        pBDTEntryEP0OutNext->STATUS.Val = _USIE|_BSTALL;
...
USB_HANDLE USBTransferOnePacket(BYTE ep,BYTE dir,BYTE* data,UINT16 len)
{
    ...
    handle->CNT = len;
    handle->STATUS.Val &= _DTSMASK;
    handle->STATUS.Val |= _USIE | _DTSEN;
...
void USBCancelIO(BYTE endpoint)
{
    if(USBPacketDisable == 1)
    {
    	...
    	pBDTEntryIn[endpoint]->STATUS.Val &= _DTSMASK;
    	pBDTEntryIn[endpoint]->STATUS.Val ^= _DTSMASK;
        #endif
    }
}
